<?php
	...
	if (isset($_POST["screenName"])) {
		if (!empty($_POST["screenName"])) {
			...
			if (strlen($screenName) > 20) {
				...
				$getFromU->update("users", $user_id, array("screen_name" => $screenName, "bio" => $bio, "country" => $country, "website" => $website));
	...
	if (isset($_FILES["profileImage"])) {
		if (!empty($_FILES["profileImage"]["name"][0])) {
			...
			$getFromU->update("users", $user_id, array("profile_image" => $fileRoot));
		}
	}
	...
	if (isset($_FILES["profileCover"])) {
		if (!empty($_FILES["profileCover"]["name"][0])) {
			...
			$getFromU->update("users", $user_id, array("profile_cover" => $fileRoot));
		}
	}
?>
					...
					<form method="post" enctype="multipart/form-data">
						<ul>
							<li>
								...
								<input id="profileImage" type="file"  name="profileImage" onchange="this.form.submit();" />
